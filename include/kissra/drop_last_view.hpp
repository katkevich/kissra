...
class drop_last_view : public TMixins... {
public:
    ...
    drop_last_view(std::size_t n, UUnderlyingView&& underlying_view)
        : curr_n(n)
        , n(n)
...
struct drop_last_mixin {
    ...
    auto drop_last(this TSelf&& self, std::size_t n) {
        ...
        return drop_last_view<std::remove_cvref_t<TSelf>, decltype(mixins)...>{ n, std::forward<TSelf>(self) };
    }
};
} // namespace kissra