...
public:
    ...
    drop_last_while_iter(TFn fn, UUnderlyingIter&& underlying_iter)
        : fn(fn)
...
struct drop_last_while_mixin {
    ...
    auto drop_last_while(this TSelf&& self, TFn fn) {
        ...
        return drop_last_while_iter<TFn, std::remove_cvref_t<TSelf>, decltype(mixins)...>{ fn, std::forward<TSelf>(self) };
    }
};
} // namespace kissra