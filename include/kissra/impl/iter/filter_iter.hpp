...
struct filter_mixin {
    ...
    constexpr auto filter(this TSelf&& self, TFn fn) {
        return with_custom_mixins<DeferInstantiation>([&]<template <typename> typename... TMixins> {
            return filter_iter<std::remove_cvref_t<TSelf>, TFn, TMixins...>{ std::forward<TSelf>(self), fn };
        });
    }
};

template <kissra::iterator_compatible T, typename TFn, typename DeferInstantiation = void>
constexpr auto filter(T&& rng_or_kissra_iter, TFn fn) {
    return impl::into_kissra_iter<DeferInstantiation>(KISSRA_FWD(rng_or_kissra_iter)).filter(fn);
}


namespace compo {
template <typename TBaseCompose, typename TFn, template <typename> typename... TMixinsCompose>
struct filter_compose : public builtin_mixins_compose<TBaseCompose>, public TMixinsCompose<TBaseCompose>... {
    ...
    constexpr auto make_iter(this TSelf&& self, UBaseIter&& base_iter) {
        return filter_iter<std::remove_cvref_t<UBaseIter>, TFn, TMixins...>{
...
struct filter_compose_mixin {
    ...
    constexpr auto filter(this TSelf&& self, TFn fn) {
        return with_custom_mixins_compose<DeferInstantiation>([&]<template <typename> typename... TMixinsCompose> {
            return filter_compose<std::remove_cvref_t<TSelf>, TFn, TMixinsCompose...>{
