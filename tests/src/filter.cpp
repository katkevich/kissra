...
TEST_CASE("filter_view::advance_tail(0) should filter out an element but not move the sentinel") {
    std::array arr = { 1, 2, 3, 4, 5 };

    auto view = kissra::all(arr).filter([](const auto& s) { return s % 2 == 0; });
    REQUIRE_EQ(*view.advance_tail(0), 4);
}
...
TEST_CASE("filter_view::advance_tail(0) should not move the sentinel") {
    std::array arr = { 1, 2, 3, 4 };
...
TEST_CASE("filter_view::next_tail() should filter out an element AND move the cursor") {
    std::array arr = { 1, 2, 3, 4, 5 };

    auto view = kissra::all(arr).filter([](const auto& s) { return s % 2 == 0; });
    REQUIRE_EQ(*view.next_tail(), 4);
    REQUIRE_EQ(*view.back(), 2);
}

TEST_CASE("filter_view::next_tail() should move the cursor") {
    std::array arr = { 1, 2, 3, 4 };
